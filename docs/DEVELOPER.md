...
Both selectors are needed, and the prefix earns its keep. It was taken off once on the theory that it was the problem, and the measurement said otherwise: the rule to beat is not only `body .et_pb_button` but also **`body.et-db #et-boc .et-l .et_pb_button`** — one id, three classes and an element — which is what Divi writes wherever a theme builder template is involved, and which a bare order class loses to. Read off the canvas with `a.matches()` against each alternative of Divi's own selector list, not reasoned about.

**And the builder's canvas was a third thing again, with a cause of its own.** The canvas does not render these modules through `FieldModuleRenderer` at all: it asks `/cscs/v1/field` for the markup, and that route rendered the field with the plugin's own renderer — which knows nothing of Divi. So the anchor there carried `cscs-button` and not `et_pb_button`, while the CSS the panel generated named both. Every setting in the Button group was correct, emitted, and one class away from matching anything. `RestPreview::field()` now asks `FieldModuleRenderer::extra_class()` for that class, so the canvas gets the markup the page gets; the class is still spelled in one place.

A warning for whoever reads styles in that canvas next: `getComputedStyle` there cannot be trusted. An inline `background-color: … !important` set on the live anchor, present in its `style` attribute, connected and painted, still read back as the old value. Selector matching (`matches()`) and the rules in `document.styleSheets` were reliable; computed values were not, and two rounds of this were spent chasing what turned out to be a stale read.
