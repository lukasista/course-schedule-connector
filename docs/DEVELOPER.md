...
This is a stopgap, on four of the five tables that use it: see the next section for the fifth, and the shape all of them are moving toward.

### Columns as children, piloted on one table

`course-schedule` — a course's own timetable — offers the shape the previous section named: columns as child modules in Divi, dragged into order in the layers panel, and inner blocks in Gutenberg. Both write into the numbered settings above rather than replacing them, so nothing downstream — `Fields::ordered_columns()`, `CourseDetail::schedule()`, the renderer — had to change, and a table with no column children, which is every one saved before this existed, behaves exactly as it always has. The two mechanisms coexist on purpose: the Columns panel is still there for anyone who prefers it, and children simply take priority the moment any exist.

`Divi\TableColumnModule` is `cscs/divi-course-schedule-column`, a hand-crafted module outside `divi/fields/` — the same way `divi/cscs-display` is — carrying one content setting, which column, and no design of its own beyond what every module already offers: a column's own typography is already declared unconditionally by the generator, keyed by column name (`{{selector}} .cscs-table .cscs-col-{$column}`), so nothing about it needed to move for this to work. It renders nothing: `TableColumnModuleDependency` registers it with a `render_callback` that always returns `''`, because it exists to be read by its parent, not to draw anything of its own. `course-schedule`'s own generated `module.json` names it in `childrenName` — the mechanism confirmed against Elegant Themes' own source, where the Accordion and the Contact Form work the same way — and every other field's `childrenName` stays empty; `tools/build-divi-modules.php`'s `module_metadata()` decides which, from a `columns_as_children` flag in the catalogue.

The Gutenberg side is `cscs/course-schedule-column`, `parent`-restricted to `cscs/course-schedule` and registered by hand in `FieldBlocks::register_column_blocks()` rather than through the generated loop, the same reasoning as the Divi module. Its `edit` is a single `SelectControl`, shown inline where it sits among its siblings in the layers list; its `save` returns `null`, the same nothing the Divi child renders — only `course-schedule`'s own `save` changes, to `InnerBlocks.Content` rather than the usual `null`, which is what asks WordPress's serialiser to keep the children in the post at all. A page saved before this exists reads back as a self-closing block with none, which is the same thing a fresh, still-empty block looks like.

Both parents read their children the same way, from the same pure helpers. `FieldModuleRenderer::render()` already read `$block->parsed_block['innerBlocks']` for the parent chain, through `BlockParserStore::get_parent()`; reading it again for the module's own children needed nothing else. `FieldBlocks`'s `render_callback` used to discard `$block` entirely (`unset( $content, $block )`), because no field needed it; it now keeps it, for this one field. `Fields::columns_from_children( $name, $inner_blocks, $child_block )` reads whichever children name a real column, in the order given, keeping only the first mention of each — Divi keeps a child's attribute nested by breakpoint and state (`column.advanced.field.desktop.value`), Gutenberg keeps it flat (`field`), and the one function reads both, so neither render path has to know the other editor's shape. `Fields::apply_children_columns( $name, $settings, $chosen )` turns that list into the `orderXxx` settings `ordered_columns()` already reads — chosen columns counting from one, in the order given, everything else sent to nought — and, handed an empty list, returns the settings untouched. That one line is the entire backward-compatibility guarantee.

The block editor's own preview of `course-schedule` no longer uses `ServerSideRender`. `WP_REST_Block_Renderer_Controller::get_item()` always rebuilds the block it renders with `'innerBlocks' => array()`, unconditionally — no parameter changes this — so a preview asked of it would never reflect a column child that had just been dragged in, only whatever the numbered settings underneath happened to say. `course-schedule` asks `/cscs/v1/field` directly instead, the same route Divi's own canvas already used, which now accepts an optional `children` parameter in the same shape `parse_blocks()` hands a render callback; the block's own `edit` reads its live children with `select( 'core/block-editor' ).getBlocks( clientId )` and sends them along with its settings on every change. Every other field is unaffected and keeps using `ServerSideRender`, which cannot see children but has none to fail to see.

Piloted on `course-schedule` alone. The other four tables — `course-makeup`, `trainer-courses`, `kind-courses`, `kind-prices` — keep their numbered-only settings for now; moving one of them the same way is a `columns_as_children` flag, a generated child module and a Gutenberg child block away, not a new mechanism. Not yet checked by hand in either Visual Builder.
...
Naming that second selector is half of it. The other half is that it takes **`{{baseSelector}}`**, not `{{selector}}` — inside a template `{{selector}}` has already had the wrapper chain spliced into it, so writing the chain again produces `body.et-db #page-container #et-boc .et-l .et-db #et-boc .et-l …`, which matches nothing all over again.

The selector is `{{selector}} .cscs-button.et_pb_button`, with nothing in front of it, and getting back to that took three wrong turns worth writing down.

`body #page-container` was bought to outrank the button styling Divi writes for the site as a whole. It is not needed. Divi splices its own wrapper chain onto a selector that begins with `{{selector}}`, and the result — `.et-db #et-boc .et-l .cscs_… .cscs-button.et_pb_button`, one id and four classes — already beats the `body.et-db #et-boc .et-l .et_pb_button` it has to beat, which is one id, three classes and an element.

What the prefix cost was the two contexts whose markup it does not describe. In a theme builder template Divi splices into a selector it did not write, and put `.et-db` *inside* `#page-container` — `.et-db` is on the body, so the rule matched nothing. Naming `customPostTypeSelector` to repair that only moved the problem: there it is `{{baseSelector}}` that gets substituted, and in the builder's canvas that resolves to a bare base order class which Divi never puts on the element. Asking the canvas `a.matches()` against each rule Divi emitted answered false for every declaration the panel had written.

**And the canvas had a second cause of its own.** It does not render these modules through `FieldModuleRenderer`: it asks `/cscs/v1/field` for the markup, and that route rendered the field with the plugin's own renderer, which knows nothing of Divi. The anchor there carried `cscs-button` and not `et_pb_button`, while the CSS named both — every setting correct, emitted, and one class away from matching anything. `RestPreview::field()` now asks `FieldModuleRenderer::extra_class()` for that class, so the canvas gets the markup the page gets and the class is still spelled in one place.

**And a third thing after those two, which the symptom named precisely.** A gradient or an image set on the button showed in the canvas; a plain colour did not. Nothing competes for `background-image`, and `background-color` was being beaten — by `body.et-db #et-boc .et-l .et_pb_button`, one id, three classes and an element, which is where Divi writes the site's own button styling. On the page Divi splices that same wrapper chain onto our selector too and ours then wins on class count; in the canvas it splices nothing, so ours stands at three classes and no id and loses every property that rule sets — the background, the radius, the weight, the size.

An id of our own is not the way out, because the only place to put one is in front and a prefix there is what breaks the splice on the page. So the groups that rule competes with — background, border, font, spacing — are marked `important` in `styleProps`, which is the same idiom Divi's own Button uses for its spacing and four of its font properties. The cost is that overriding a button's colour from a stylesheet now needs `!important` of its own; the alternative was a panel that works in one context out of two.

Both halves were needed, and both were confirmed in the canvas: the rules Divi emits now answer true to `a.matches()`, and the button draws with the radius and weight the panel was given.

A warning for whoever reads styles in that canvas next: **`getComputedStyle` there cannot be trusted.** An inline `background-color: … !important` set on the live anchor, present in its `style` attribute, on a connected and painted element, still read back as the old value. Selector matching, the rules in `document.styleSheets`, and a screenshot were reliable; computed values were not, and two rounds were spent chasing a stale read — including one reversal of a correct fix.
...
| `cscs_cancelled_course_redirect` | Where the address of a withdrawn course sends a visitor. Defaults to the page of the kind it belonged to; an empty string leaves the not-found page as it is |
