...
Both of those went away again, and the lesson is the one underneath them: **the prefix was the problem, not the splice.** `body #page-container` was bought to outrank the button styling Divi writes for the site as a whole, and it was not buying much — the rule it has to beat is `body .et_pb_button`, one class and one element, which an order class with two classes after it beats with room to spare. What it cost was every context whose markup the prefix does not describe: a theme builder template, where Divi splices its own wrappers into a selector it did not write, and the builder's canvas. The selector is now `{{selector}} .cscs-button.et_pb_button` and nothing else, which is how every other part of these modules is named — and those are the parts that have worked in all three places all along.
