...
- **`Listing`** is what a template receives, with every decision already made — cell text, cell markup, row classes, the wording for empty and full. A template that only arranges things cannot break a rule by being rewritten.
