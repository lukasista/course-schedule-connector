...
| **F3b** | Administrace – obsah | **Zobrazovací sady (hotovo)**, editace kurzu (obohacený obsah, kontakt na lektora, příznak přihlášek, zámky polí), **ruční kurzy (hotovo)** zakládané ručně i jedním kliknutím z nespárovaného termínu, obrazovka nespárovaných lekcí, **obrazovka náhradních lekcí (hotovo)**, **editace kurzu se zámky polí a kontaktem na lektora (hotovo)**, **obrazovka nespárovaných lekcí (hotovo)**, **hromadné nastavení tlačítka (hotovo)**, **mapování sálů (hotovo)**, hromadné akce | 1,5 dne |
| **F4** | Renderer + styly | ✅ *hotovo* – jeden renderer pro všechny výstupy, šablony přepsatelné v tématu, responzivní překlopení tabulek podle nastavené šířky, stavy (obsazeno, zrušeno, poslední místa), formátování ceny `1 960 Kč`, tlačítko ve třech úrovních, `[cscs_courses]` a `[cscs_schedule]` | 1,5 dne |
| **F5** | Shortcode + Gutenberg blok | ✅ *hotovo* – nezávislost na Divi: `[cscs_courses]`, `[cscs_schedule]`, blok `cscs/display` s jediným polem (výběr sady), serverový náhled v editoru přes blokový endpoint, bez build kroku | 1 den |
| **F6** | Divi 5 modul | ✅ *hotovo* – jeden modul `cscs/divi-display` s jediným obsahovým polem (výběr sady), `module.json`, PHP render přes `Module::render()`, náhled ve VB přes REST, **bez webpack buildu** (plain JS proti globálům Divi), oddělení obsahu a designu **vynucené při ukládání**, ne skrýváním polí | 3 dny |
| **F7** | Detail kurzu + frontend | ✅ *hotovo* – stránka detailu kurzu (obsah, fakta, kontakt na lektora, tlačítko, rozvrh kurzu, náhradní lekce u kurzu), šablona přepsatelná v tématu, JSON-LD `Course`, přepínač týdnů, filtr sálu a stránkování — odkazy fungují i bez JavaScriptu, skript je jen zrychluje | 2 dny |
