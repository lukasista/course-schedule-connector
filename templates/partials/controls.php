...
if ( ! $cscs_listing->has_weeks() && array() === $cscs_listing->rooms ) {
	return;
}

$cscs_rooms_id = 'cscs-rooms-' . $cscs_listing->set->id;

?>
<div class="cscs-controls">
	<?php if ( $cscs_listing->has_weeks() ) : ?>
		<nav class="cscs-weeks" aria-label="<?php esc_attr_e( 'Which week', 'course-schedule-connector' ); ?>">
			<a class="cscs-weeks__step" data-cscs-nav="week" data-cscs-value="<?php echo esc_attr( (string) ( $cscs_listing->args->week - 1 ) ); ?>" href="<?php echo esc_url( $cscs_listing->url( 'week', $cscs_listing->args->week - 1 ) ); ?>" rel="prev">
				<?php esc_html_e( '← Previous week', 'course-schedule-connector' ); ?>
			</a>

			<span class="cscs-weeks__current"><?php echo esc_html( $cscs_listing->week_label() ); ?></span>

			<?php if ( 0 !== $cscs_listing->args->week ) : ?>
				<a class="cscs-weeks__today" data-cscs-nav="week" data-cscs-value="0" href="<?php echo esc_url( $cscs_listing->url( 'week', 0 ) ); ?>">
					<?php esc_html_e( 'This week', 'course-schedule-connector' ); ?>
				</a>
			<?php endif; ?>

			<a class="cscs-weeks__step" data-cscs-nav="week" data-cscs-value="<?php echo esc_attr( (string) ( $cscs_listing->args->week + 1 ) ); ?>" href="<?php echo esc_url( $cscs_listing->url( 'week', $cscs_listing->args->week + 1 ) ); ?>" rel="next">
				<?php esc_html_e( 'Next week →', 'course-schedule-connector' ); ?>
			</a>
		</nav>
	<?php endif; ?>

	<?php if ( array() !== $cscs_listing->rooms ) : ?>
		<form class="cscs-rooms" method="get" action="<?php echo esc_url( $cscs_listing->form_action() ); ?>">
			<?php foreach ( $cscs_listing->hidden_fields() as $cscs_field => $cscs_value ) : ?>
			...
			<select class="cscs-rooms__select" id="<?php echo esc_attr( $cscs_rooms_id ); ?>" name="cscs_room" data-cscs-nav="room">
				...
				<?php foreach ( $cscs_listing->rooms as $cscs_room_id => $cscs_room_name ) : ?>
					<option value="<?php echo esc_attr( (string) $cscs_room_id ); ?>" <?php selected( $cscs_listing->args->room, (int) $cscs_room_id ); ?>>
