...
- Make-up lessons can be tied to the course they stand in for. Nothing in the data says which course that is — the timetable calls one "Náhradní lekce 4-6 let" and several courses run for that age group — so a person records it, once per activity name rather than once per occurrence, since the same lesson repeats weekly. `wp cscs makeup list|link`.
